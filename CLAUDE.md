...
## Мова спілкування (КРИТИЧНО)

Завжди відповідай користувачу тільки українською мовою.
...
- **Ніколи не використовуй `useSearchParams()` на статичних/ISR-сторінках** ("●" в `next build`), навіть у клієнтських компонентах з кореневого layout (напр. `MetaPixel`). Хук форсує весь Suspense-бавундарі сторінки в client-only рендер під час статичної генерації — non-JS краулери (Meta/Facebook, Google без виконання JS) бачать лише `loading.tsx`-скелетон замість реального контенту. Якщо треба реагувати на зміну query-параметра (напр. `?sort=`), читай `window.location.search` через `useSyncExternalStore` (server snapshot = дефолт, client snapshot = реальне значення) — так само, як `Header.tsx` читає кошик з localStorage. Якщо треба лише реагувати на зміну маршруту — досить `usePathname()`. Перевірка регресії: `next build` + `next start` + `curl -A "facebookexternalhit/1.1" -L <url> | grep BAILOUT_TO_CLIENT_SIDE_RENDERING` — має бути 0 збігів на `●`-сторінках.
